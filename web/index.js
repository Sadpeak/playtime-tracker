...
app.get('/search', (req, res) => {
   ...
   if(req.query.steamid){
   const querryT = `SELECT steamid, name, GROUP_CONCAT(DISTINCT name SEPARATOR '\n') AS names, SUM(end - start) AS total, COUNT(*) AS sessions, GROUP_CONCAT(DISTINCT flags SEPARATOR '|') AS flags FROM \`playtime_tracker\` WHERE steamid = "${req.query.steamid}"`
